...
{
    ros::init(argc, argv, "baxter_right_arm_mocap_servoing_controller");
    ROS_INFO("Starting baxter_right_arm_mocap_servoing_controller...");
    ros::NodeHandle nh;
    ros::NodeHandle nhp("~");
    ...
    nhp.param(std::string("arm_pose_topic"), arm_pose_topic, std::string("/right_arm_pose_controller/pose"));
    ...
    nhp.param(std::string("arm_controller_state"), arm_controller_state, std::string("/right_arm_pose_controller/state"));
    ...
    if (arm_pose_topic == std::string(""))
    {
        ...
        baxter_mocap_servoing::MocapServoingController controller(nh, std::string("right_arm"), target_pose_topic, robot_config_topic, arm_command_action, arm_controller_state, abort_service, kp, ki, kd);
    ...
    else
    {
        ...
        baxter_mocap_servoing::MocapServoingController controller(nh, std::string("right_arm"), arm_pose_topic, target_pose_topic, robot_config_topic, arm_command_action, arm_controller_state, abort_service, kp, ki, kd);
        ROS_INFO("...startup complete");
        controller.Loop();
    }
    return 0;
